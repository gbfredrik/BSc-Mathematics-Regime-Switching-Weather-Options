...
DAT = retime(Sets(1, 1).CleanSet(:,1), 'daily', 'mean');
...
temp = transpose(DAT.Degrees(24000:25839,:));

[X, FVAL] = fmincon(@(x) sum((temp - SeasonFunc(x(1), x(2), x(3), x(4), 0:1839)).^2), [18;0.005;5;0])

%%
figure(1)
hold on
plot(SeasonFunc(X(1), X(2), X(3), X(4), 0:1839), 'r', 'LineWidth', 3)
plot(temp, 'b')
plot(SeasonFunc(X(1), X(2), 0, 0, 0:1839))
plot(18 .* ones(1840, 1), 'g', 'LineWidth', 2)
hold off

figure(2)
hold on
subplot(2,1,1)
plot(temp - SeasonFunc(X(1), X(2), X(3), X(4), 0:1839))

subplot(2,1,2)
histogram(temp - SeasonFunc(X(1), X(2), X(3), X(4), 0:1839))
hold off
